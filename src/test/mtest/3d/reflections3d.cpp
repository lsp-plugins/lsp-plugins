...
namespace mtest
{
    ...
    typedef struct global_context_t
    {
        ...
        cstorage<v_triangle3d_t>    traced;     // List of traced triangles (for debug)
...
//    }
     ...
     *
     * The strict mode performs strict splitting of trianlges as an opposite to relaxed mode
     *
     ...
     */
    static void split_triangle_strict(
            v_triangle3d_t *out,
            size_t *n_out,
            v_triangle3d_t *in,
            size_t *n_in,
                ...
                *n_q        = 1;
                ...
                for (size_t k=0; ; )
                {
                    ...
                    *n_in   = 0;
                    ...
                    {
                        split_triangle_relaxed(out, &n_out, in, n_in, &pl[k], &q[l]);
                        if ((n_out > 16) || ((*n_in) > 16))
...
#define TRACE_SKIP(ctx, action) \
        ...
        )
    {
        ...
        n_out = 0, n_in = 0;
        split_triangle_strict(out, &n_out, in, &n_in, pl, &t);
        if ((n_out > 2) || (n_in > 2))
            lsp_trace("split overflow: n_out=%d, n_in=%d", int(n_out), int(n_in));
        ...
        TRACE_BREAK(ctx,
            ...
            for (size_t i=0; i<n_in; ++i)
                ctx->global->view->add_triangle_1c(&in[i], (i) ? &C_BLUE : &C_CYAN);
        );
        // END STEP
        ...
        {
            n_sin = 0, n_sout = 0;
            split_triangle_relaxed(sout, &n_sout, sin, &n_sin, pl, source.at(i));
            if ((n_sout > 2) || ((n_sin) > 2))
        ...
        if (n_in == 1)
        {
            ...
            if (n_out == 2) // in=1, out=2
            {
                ...
                {
                    n_sin = 0, n_sout = 0;
                    split_triangle_relaxed(sout, &n_sout, sin, &n_sin, &npl, clipped.at(i));
                    if ((n_sout > 2) || ((n_sin) > 2))
                        lsp_trace("split overflow: n_sout=%d, n_sin=%d", int(n_sout), int(n_sin));
        ...
        else // in=2, out=1
        {
            ...
            {
                n_sin = 0, n_sout = 0;
                split_triangle_relaxed(sout, &n_sout, sin, &n_sin, &npl, source.at(i));
                if ((n_sout > 2) || ((n_sin) > 2))
                    lsp_trace("split overflow: n_sout=%d, n_sin=%d", int(n_sout), int(n_sin));
        ...
        TRACE_BREAK(ctx,
            ...
            if (sctx1 != NULL)
            {
                sctx1->global->view->add_triangle_pv1c(sctx1->front.p, &C_GREEN);
                for (size_t i=0, n=sctx1->source.size(); i<n; ++i)
                    sctx1->global->view->add_triangle_1c(sctx1->source.at(i), &C_GREEN);
            }
            if (sctx2 != NULL)
            {
                sctx2->global->view->add_triangle_pv1c(sctx2->front.p, &C_BLUE);
                for (size_t i=0, n=sctx2->source.size(); i<n; ++i)
                    sctx2->global->view->add_triangle_1c(sctx2->source.at(i), &C_BLUE);
            }
        );
        ...
        )
    {
        ...
        for (size_t i=0, n=source.size(); i<n; ++i)
        {
            ...
            // Split all triangles:
            split_triangle_relaxed(out, &n_out, buf1, &n_buf1, pl, &t);
        ...
        calc_plane_vector_p3(&spl[0], &ctx->front.s, &ctx->front.p[0], &ctx->front.p[1]);
        ...
        *q          = *ct;
        ...
        for (size_t k=0; ; )
        {
            ...
            *n_in   = 0;
            ...
            for (size_t l=0; l < *n_q; ++l)
            {
                split_triangle_relaxed(out, &n_out, in, n_in, &spl[k], &q[l]);
                if ((n_out > 16) || (*n_in > 16))
                    lsp_trace("split overflow: n_out=%d, n_in=%d", int(n_out), int(*n_in));
            }
        ...
        TRACE_BREAK(ctx,
            for (size_t i=0, n=ctx->source.size(); i<n; ++i)
                ctx->global->view->add_triangle_3c(ctx->source.at(i), &C_CYAN, &C_MAGENTA, &C_YELLOW);
    ...
    class Renderer: public X11Renderer
    {
        ...
        protected:
            status_t    update_view()
            {
                ...
                status_t res = prepare_scene(global.ignored, global.scene, pScene);
                if (res != STATUS_OK)
                {
                    tasks.flush();
                    delete ctx;
                ...
                {
                    v_triangle3d_t *t = global.traced.at(i);
                    v[0].p     = t->p[0];
                    v[0].n     = t->n[0];
                    v[0].c     = C_CYAN;
