...
namespace lsp
{
    ...
    void compressor_base::update_settings()
    {
        ...
        for (size_t i=0; i<channels; ++i)
        {
            ...
            c->sSC.set_mode((c->pMode != NULL) ? c->pScMode->getValue() : SCM_RMS);
...
//#ifdef LSP_TRACE
//    static void dump_buffer(const char *comment, const float *x, size_t count)
//    {
//        char buf[4096];
//        lsp_trace("%s:", comment);
//        lsp_trace("------------------------------------------------------------\n");
...
//        {
//            char *p = buf;
//            for (size_t i=0; (i<32) && (count > 0); ++i, --count)
//                p += sprintf(p, "%f ", *(x++));
//
//            printf("%s\n", buf);
//        }
...
//    }
//#else
//    #define dump_buffer(comment, x, count)
//#endif /* LSP_TRACE */
