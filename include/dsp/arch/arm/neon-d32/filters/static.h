...
namespace neon_d32
{
    ...
    void biquad_process_x4(float *dst, const float *src, size_t count, biquad_t *f)
    {
        ...
        (
            ...
            __ASM_EMIT("vmla.f32    q10, q3, q0")                           // q10   = s2 = d0 + a0*s
            ...
            __ASM_EMIT("vext.32     q8, q9, $1")                            // q8    = (vmask << 1) | 1
            __ASM_EMIT("orr         %[imask], %[imask], LSL $1")            // imask = (imask << 1) | imask
            __ASM_EMIT("subs        %[count], $1")
            __ASM_EMIT("beq         4f")
            __ASM_EMIT("cmp         %[imask], $0x0f")
            __ASM_EMIT("bne         1b")
            ...
            __ASM_EMIT("5:")
            __ASM_EMIT("lsl         %[imask], $1")                          // imask = imask << 1
            __ASM_EMIT("vext.32     q8, q9, $1")                            // q8    = (vmask << 1) | 0
            __ASM_EMIT("vmla.f32    q10, q3, q0")                           // q10   = s2 = d0 + a0*s
            __ASM_EMIT("vmul.f32    q11, q4, q0")                           // q11   = a1*s
            __ASM_EMIT("vmul.f32    q12, q5, q0")                           // q12   = a2*s
            __ASM_EMIT("vmla.f32    q11, q6, q1")                           // q11   = a1*s + b1*s2
            ...
            __ASM_EMIT("vbit        q1, q11, q8")                           // q1    = (d0 & ~vmask) | (d0' & vmask)
            __ASM_EMIT("vstm        %[dst]!, {s3}")
            __ASM_EMIT("tst         %[imask], $0x0f")                       // mask  == 0 ?
            __ASM_EMIT("bne         5b")
