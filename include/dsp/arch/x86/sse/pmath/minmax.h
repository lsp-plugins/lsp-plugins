...
namespace sse
{
    ...
    #define ABS_MINMAX_CORE(DST, A, B, CMP) \
        __ASM_EMIT("movaps      %[X_SIGN], %%xmm6") \
        __ASM_EMIT("xor         %[off], %[off]") \
        __ASM_EMIT("movaps      %%xmm6, %%xmm7") \
        __ASM_EMIT("sub         $8, %[count]") \
        ...
        __ASM_EMIT("andps       %%xmm6, %%xmm0")            /* xmm0 = abs(a) */ \
        __ASM_EMIT("andps       %%xmm7, %%xmm1") \
        __ASM_EMIT("andps       %%xmm6, %%xmm2")            /* xmm2 = abs(b) */ \
        __ASM_EMIT("andps       %%xmm7, %%xmm3") \
        __ASM_EMIT("movaps      %%xmm0, %%xmm4")            /* xmm4 = abs(a) */ \
        __ASM_EMIT("movaps      %%xmm1, %%xmm5") \
        __ASM_EMIT("cmpps     " CMP ", %%xmm2, %%xmm4")     /* xmm4 = abs(a) <=> abs(b) */ \
        __ASM_EMIT("cmpps     " CMP ", %%xmm3, %%xmm5") \
        __ASM_EMIT("andps       %%xmm4, %%xmm0")            /* xmm0 = a & (abs(a) <=> abs(b)) */ \
        __ASM_EMIT("andps       %%xmm5, %%xmm1") \
        __ASM_EMIT("andnps      %%xmm2, %%xmm4")            /* xmm4 = b & ~(abs(a) <=> abs(b)) */ \
        __ASM_EMIT("andnps      %%xmm3, %%xmm5") \
        __ASM_EMIT("orps        %%xmm4, %%xmm0")            /* xmm0 = (a & (abs(a) <=> abs(b))) | (b & ~(abs(a) <=> abs(b))) */ \
        ...
        __ASM_EMIT("movups      0x00(%[" B "], %[off]), %%xmm2") \
        __ASM_EMIT("andps       %%xmm6, %%xmm0")            /* xmm0 = abs(a) */ \
        __ASM_EMIT("andps       %%xmm7, %%xmm2")            /* xmm2 = abs(b) */ \
        __ASM_EMIT("movaps      %%xmm0, %%xmm4")            /* xmm4 = abs(a) */ \
        __ASM_EMIT("cmpps     " CMP ", %%xmm2, %%xmm4")     /* xmm4 = abs(a) <=> abs(b) */ \
        __ASM_EMIT("andps       %%xmm4, %%xmm0")            /* xmm0 = a & (abs(a) <=> abs(b)) */ \
        __ASM_EMIT("andnps      %%xmm2, %%xmm4")            /* xmm4 = b & ~(abs(a) <=> abs(b)) */ \
