...
#define FFT_BUTTERFLY_BODY8(add_b, add_a) \
    ...
    ( \
        ...
        __ASM_EMIT("vmulps      %%ymm6, %%ymm2, %%ymm4")            /* ymm4 = x_re * b_re */ \
        __ASM_EMIT("vmulps      %%ymm6, %%ymm3, %%ymm5")            /* ymm5 = x_re * b_im */ \
        __ASM_EMIT("vmulps      %%ymm7, %%ymm2, %%ymm2")            /* ymm2 = x_im * b_re */ \
        __ASM_EMIT("vmulps      %%ymm7, %%ymm3, %%ymm3")            /* ymm3 = x_im * b_im */ \
        __ASM_EMIT(add_b "      %%ymm2, %%ymm5, %%ymm5")            /* ymm5 = c_re = x_re * b_im -+ x_im * b_re */ \
        __ASM_EMIT(add_a "      %%ymm3, %%ymm4, %%ymm4")            /* ymm4 = c_im = x_re * b_re +- x_im * b_im */ \
        /* Perform butterfly */ \
        ...
        : __IF_32([ptr1] "=&r" (ptr1), [ptr2] "=&r" (ptr2), ) \
          [off_a] "+r" (off_a), [off_b] "+r" (off_b), [np] __ASM_ARG_RW(np) \
...
namespace avx
{
    static inline void butterfly_direct4(float *dst_re, float *dst_im, size_t rank, size_t blocks)
    {
        size_t pairs = 1 << rank;
        rank = (rank - 2) << 4;
