 ...
 *      Author: sadko
 */
#ifndef CORE_X86_SSE_FASTCONV_H_IMPL
    #error "This header should not be included directly"
#endif /* CORE_X86_SSE_FASTCONV_H_IMPL */

namespace lsp
{
    namespace sse
    {
        static inline void APPLY_IMPL(float *tmp, const float *c1, const float *c2, size_t rank)
        {
...
//                __ASM_EMIT(".align 16")
                ...
                // Load data
                __ASM_EMIT(MV_C1 "      0x00(%[c1]), %%xmm0")       /* xmm0 = r0 r1 r2 r3 */
                __ASM_EMIT(MV_C1 "      0x10(%[c1]), %%xmm2")       /* xmm1 = i0 i1 i2 i3 */
                __ASM_EMIT(MV_C1 "      0x20(%[c1]), %%xmm4")
                __ASM_EMIT(MV_C1 "      0x30(%[c1]), %%xmm6")
                ...
                // xmm4 = r4 r5 r6 r7
                // xmm6 = i4 i5 i6 i7
                __ASM_EMIT(MV_C2 "      0x00(%[c2]), %%xmm1")       /* xmm1 = rc0 rc1 rc2 rc3 */
                __ASM_EMIT(MV_C2 "      0x10(%[c2]), %%xmm3")       /* xmm3 = ic0 ic1 ic2 ic3 */
                ...
                __ASM_EMIT(MV_C2 "      0x20(%[c2]), %%xmm0")       /* xmm0 = rc4 rc5 rc6 rc7 */
                __ASM_EMIT(MV_C2 "      0x30(%[c2]), %%xmm2")       /* xmm2 = ic4 ic5 ic6 ic7 */
                ...
                __ASM_EMIT(MV_TMP "     %%xmm0, 0x00(%[tmp])")
                __ASM_EMIT(MV_TMP "     %%xmm1, 0x10(%[tmp])")
                __ASM_EMIT(MV_TMP "     %%xmm4, 0x20(%[tmp])")
                __ASM_EMIT(MV_TMP "     %%xmm5, 0x30(%[tmp])")

                __ASM_EMIT("add         $0x40, %[tmp]")
                __ASM_EMIT("add         $0x40, %[c1]")
                __ASM_EMIT("add         $0x40, %[c2]")
                __ASM_EMIT("sub         $16, %[k]")
                __ASM_EMIT("jnz         1b")

                : [tmp] "+r" (tmp), [k] "+r" (items), [c1] "+r" (c1), [c2] "+r" (c2)
                :
                : "cc", "memory",
                  "%xmm0", "%xmm1", "%xmm2", "%xmm3",
                  "%xmm4", "%xmm5", "%xmm6", "%xmm7"
