...
void test_matched_transform_x1(biquad_x1_t *bf, f_cascade_t *bc, float kf, float td, size_t count)
{
    ...
    float RE[4], IM[4], AT[4], AB[4];
    float AN[4], N2[4], N1[4];

    COSW[0] = cos(w);
    COSW[1] = COSW[0]; COSW[2] = COSW[0]; COSW[3] = COSW[0];

    SINW[0] = sin(w);
    SINW[1] = SINW[0]; SINW[2] = SINW[0]; SINW[3] = SINW[0];

    for (size_t i=0; i<4; ++i)
    {
        COS2W[i]    = COSW[i]*COSW[i] - SINW[i]*SINW[i];
        SIN2W[i]    = 2*SINW[i]*COSW[i];
    }
    ...
    {
        // Transpose top part
        T0[0] = bc[0].t[0]; T0[1] = bc[1].t[0]; T0[2] = bc[2].t[0]; T0[3] = bc[3].t[0];
        T1[0] = bc[0].t[1]; T1[1] = bc[1].t[1]; T1[2] = bc[2].t[1]; T1[3] = bc[3].t[1];
        T2[0] = bc[0].t[2]; T2[1] = bc[1].t[2]; T2[2] = bc[2].t[2]; T2[3] = bc[3].t[2];
        T3[0] = bc[0].t[3]; T3[1] = bc[1].t[3]; T3[2] = bc[2].t[3]; T3[3] = bc[3].t[3];
        ...
        for (size_t i=0; i<4; ++i)
        {
            RE[i]       = T0[i]*COS2W[i] + T1[i]*COSW[i] + T2[i];
            IM[i]       = T0[i]*SIN2W[i] + T1[i]*SINW[i];
            AT[i]       = sqrtf(RE[i]*RE[i] + IM[i]*IM[i]);
        }
        ...
        B0[0] = bc[0].b[0]; B0[1] = bc[1].b[0]; B0[2] = bc[2].b[0]; B0[3] = bc[3].b[0];
        B1[0] = bc[0].b[1]; B1[1] = bc[1].b[1]; B1[2] = bc[2].b[1]; B1[3] = bc[3].b[1];
        B2[0] = bc[0].b[2]; B2[1] = bc[1].b[2]; B2[2] = bc[2].b[2]; B2[3] = bc[3].b[2];
        B3[0] = bc[0].b[3]; B3[1] = bc[1].b[3]; B3[2] = bc[2].b[3]; B3[3] = bc[3].b[3];
        ...
        for (size_t i=0; i<4; ++i)
        {
            RE[i]       = B0[i]*COS2W[i] + B1[i]*COSW[i] + B2[i];
            IM[i]       = B0[i]*SIN2W[i] + B1[i]*SINW[i];
            AB[i]       = sqrtf(RE[i]*RE[i] + IM[i]*IM[i]);
        }
        ...
        for (size_t i=0; i<4; ++i)
        {
            ...
            bf[i].b[0]    = -B1[i] * N2[i]; // Sign negated
            bf[i].b[1]    = -B2[i] * N2[i]; // Sign negated
            bf[i].b[2]    = 0.0f;
            bf[i].b[3]    = 0.0f;
        }
